...
describe('E2E command and CI safety', () => {
  it('uses a viable default amount without relaxing the protocol fee cap', async () => {
    ...
    assert.match(runner, /withDefault\('E2E_READ_AMOUNT', '100000000000000000'\)/)
    assert.match(example, /^E2E_READ_AMOUNT=100000000000000000$/m)
  ...
  it('keeps the pull-request workflow read-only and secret-free', async () => {
    ...
    assert.match(workflow, /E2E_READ_AMOUNT: \$\{\{ vars\.E2E_READ_AMOUNT \|\| '100000000000000000' \}\}/)
