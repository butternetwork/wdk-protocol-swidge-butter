...
import { BTC_CHAIN_ID, SOLANA_CHAIN_ID, SYMBOLIC_NATIVE_TOKEN_IDS, TRON_CHAIN_ID } from './constants.js';
/**
...
const CHAIN_NATIVE_SYMBOL = new Map([
    [BTC_CHAIN_ID, 'btc'],
    [SOLANA_CHAIN_ID, 'sol'],
    [TRON_CHAIN_ID, 'trx']
]);
...
export function normalizeTokenIdentifier(chainId, value) {
    ...
    if (lowered === 'native' || lowered === ZERO_ADDRESS || lowered === NATIVE_SENTINEL)
        return `native:${chain}`;
    if (lowered === CHAIN_NATIVE_SYMBOL.get(chain))
        return `native:${chain}`;
    if (chain === SOLANA_CHAIN_ID && trimmed === SOLANA_NATIVE_TOKEN)
        return `native:${chain}`;
    if (chain === TRON_CHAIN_ID) {
        const account = tronAccountId(trimmed);
        if (account === ZERO_ADDRESS.slice(2))
            return `native:${chain}`;
        if (account != null)
            return `tron:${account}`;
    }
...
export function isSymbolicNativeTokenIdentifier(chainId, token) {
    ...
    if (!SYMBOLIC_NATIVE_TOKEN_IDS.has(lowered))
        return false;
...
export function toButterTokenIdentifier(chainId, token) {
    ...
    if (normalizeTokenIdentifier(chain, trimmed) !== `native:${chain}`)
        return trimmed;
    if (chain === SOLANA_CHAIN_ID)
        return SOLANA_NATIVE_TOKEN;
    if (chain === TRON_CHAIN_ID)
        return TRON_NATIVE_TOKEN;
    if (chain === BTC_CHAIN_ID)
        return ZERO_ADDRESS;
    return trimmed;
}
/**
...
export function normalizeTransactionHash(value) {
    const trimmed = value.trim();
    if (EVM_HEX.test(trimmed) || BARE_HEX_TX_HASH.test(trimmed))
 ...
 * @param {string | undefined} left - The first value to compare.
 * @param {string | undefined} right - The second value to compare.
 * @returns {boolean} Whether normalization produces the same non-empty identifier for both values.
 */
 ...
 *
 * @param {string} token - The Tron Base58Check, `41`-hex, or `0x` address to decode.
...
function tronAccountId(token) {
    ...
    if (butterHex?.[1])
        return butterHex[1].toLowerCase();
    const tronHex = /^(?:0x)?41([0-9a-fA-F]{40})$/.exec(token);
    if (tronHex?.[1])
        return tronHex[1].toLowerCase();
    try {
